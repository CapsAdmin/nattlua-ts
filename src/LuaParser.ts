import { BaseParser, ParserNode } from "./BaseParser.ts";
import { BaseSyntax } from "./BaseSyntax.ts";
import { Code } from "./Code.ts";
import { LuaLexer, syntax, syntax_typesystem } from "./LuaLexer.ts";
...
export class LuaParser extends BaseParser<AnyParserNode> {
  ...
  ReadSubExpression(primary: PrimaryExpressionNode) {
    ...
    for (let i = 0; i < this.GetLength(); i++) {
      ...
      if (
        this.IsValue(":") &&
        (!this.IsType("letter", 1) || !this.IsCallExpression(2))
      ) {
      ...
      const sub = this.ReadIndexSubExpression() ||
  ...
  ReadLocalFunctionStatement() {
    if (!(this.IsValue("local") && this.IsValue("function", 1))) {
      return undefined;
    }
  ...
  ReadLocalAnalyzerFunctionStatement() {
    if (
      !(this.IsValue("local") && this.IsValue("analyzer", 1) &&
        this.IsValue("function", 2))
    ) {
      return undefined;
    }
  ...
  ReadReturnStatement() {
    ...
    node.expressions = this.ReadMultipleValues(
      undefined,
      () => this.ReadExpression(0),
      node.Tokens[","],
    ); // TODO: avoid creating closure
  ...
  ReadDestructureAssignmentStatement() {
    ...
    node.Tokens["{"] = this.ExpectValue("{");
    node.left = this.ReadMultipleValues(
      undefined,
      () => this.ExpectIdentifier(),
      node.Tokens[","],
    );
    ...
    if (
      !(
        ...
        ((this.IsValue("{", 1) && this.IsType("letter", 2)) ||
          (this.IsType("letter", 1) && this.IsValue(",", 2) &&
            this.IsValue("{", 3)))
      )
    ) {
      return undefined;
  ...
  ReadCallOrAssignmentStatement() {
    ...
    const left = this.ReadMultipleValues(
      Infinity,
      () => this.ReadExpression(0),
      left_commas,
    );
    ...
    if (this.IsValue("=")) {
      ...
      node.left = left;
      node.Tokens["right,"] = [];
      node.right = this.ReadMultipleValues(
        Infinity,
        () => this.ReadExpression(0),
        node.Tokens["right,"],
      );
    ...
    if (
      left[0] && (left[0].Kind == "postfix_call" || left[0].Kind == "import") &&
      !left[1]
    ) {
  ...
  ReadLocalTypeFunctionStatement() {
    if (
      !(this.IsValue("local") && this.IsValue("type", 1) &&
        this.IsValue("function", 2))
    ) {
      return undefined;
    }
  ...
  ReadTypeAssignmentStatement() {
    if (
      !(this.IsValue("type") &&
        (this.IsType("letter", 1) || this.IsValue("^", 1)))
    ) {
      return;
    }
  ...
  ReadIndexExpression() {
    let node = this.StartNode("expression", "value") as
      | BinaryOperatorExpression
      | ValueExpression;
    if (node.Kind == "value") {
  ...
  ReadTableExpression() {
    ...
    for (let i = 0; i < this.GetLength(); i++) {
      ...
      if (
        entry.Kind == "table_index_value" || entry.Kind == "table_key_value"
      ) {
        if (entry.spread) {
          tree.spread = true;
        }
  ...
  private CheckIntegerDivision(token: Token) {
    if (!token.integer_division_resolved && token.whitespace) {
      for (let i = 0; i < token.whitespace.length; i++) {
        ...
        if (
          whitespace.type == "line_comment" &&
          whitespace.value.substring(0, 2) == "//"
        ) {
          token.whitespace.splice(i, 1);

          const tokens = new LuaLexer(
            new Code("/idiv" + whitespace.value.substring(1)),
          ).GetTokens();
  ...
  >(
    ...
    if (primary) {
      ...
      if (sub) {
        if (
          primary.Kind == "value" &&
          (primary.value.type == "letter" || primary.value.value == "...")
        ) {
    ...
    while (node) {
      ...
      node.right = this.ExpectExpression(
        syntax.GetBinaryOperatorInfo(node.operator)!.right_priority,
      )!;
