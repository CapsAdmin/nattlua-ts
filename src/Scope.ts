import { Environment } from "./Analyzer"
...
export class LexicalScope {
	...
	FindValue(key: string, env: Environment) {
		...
		while (scope) {
			...
			if (upvalue) {
				const upvalue_position = prev_scope ? prev_scope.upvalue_position : undefined
	...
	constructor(public parent: LexicalScope | undefined, public upvalue_position: number) {
		this.ref = ref++
