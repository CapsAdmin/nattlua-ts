...
export class LuaEmitter extends BaseEmitter {
  ...
  EmitExpression(node: AnyExpressionNode) {
    ...
    if (node.Kind == "value") {
      ...
      node.Kind == "function" || node.Kind == "analyzer_function" ||
      node.Kind == "type_function"
    ) {
      this.EmitFunction(node);
  ...
  EmitDestructureAssignment(
    node: AssignmentLocalDestructureStatement | AssignmentDestructureStatement,
  ) {
    this.Whitespace("\t");
  ...
  EmitLocalAssignment(
    node: AssignmentLocalStatement | AssignmentLocalTypeStatement,
  ) {
    this.EmitToken(node.Tokens["local"]);
  ...
  EmitFunction(
    ...
    if (
      node.Kind == "local_analyzer_function" || node.Kind == "analyzer_function"
    ) {
