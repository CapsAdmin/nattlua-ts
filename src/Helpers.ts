...
export class Helpers {
  static LinePositionToSubPosition(
    code: string,
    line: number,
    character: number,
  ): number {
  ...
  static FormatError(
    ...
    const { line: startLine, character: startCharacter } = Helpers
      .SubPositionToLinePosition(code_string, start);
    const { line: stopLine, character: stopCharacter } = Helpers
      .SubPositionToLinePosition(code_string, stop);
